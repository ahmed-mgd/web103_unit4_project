...
import { getBuildById, updateBuild } from '../services/BuildsAPI'
...
const EditCar = () => {
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const [allComps, build] = await Promise.all([
          getAllComponents(),
          getBuildById(id)
        ])
        ...
        // Pre-select saved components
        const findComp = (type, compId) =>
          allComps.find(c => c.type === type && c.id === compId) || null

        setSelected({
          cpu:         findComp('cpu', build.cpu_id),
          motherboard: findComp('motherboard', build.motherboard_id),
          gpu:         findComp('gpu', build.gpu_id),
          ram:         findComp('ram', build.ram_id),
          storage:     findComp('storage', build.storage_id),
          case:        findComp('case', build.case_id),
        })
      } catch (err) {
        console.error(err)
      } finally {
    ...
    <div className="builder-page">
      <div className="builder-left">
        <div className="visualizer-card">
          <div className="edit-back-row">
            <Link to={`/customcars/${id}`} className="back-link">← Back to Details</Link>
          </div>
      ...
      <div className="builder-right">
        <div className="builder-header">
          <h1 className="builder-title">Edit Build</h1>
          <p className="builder-subtitle">Update your component selections</p>
        </div>
